...
2. **Configure Environment (Optional)**: Copy =dockerize/.env.template= to =dockerize/.env= and fill in your configuration.
   #+begin_src bash
...
*** Dockerize Scripts (run from =dockerize/=, or use =pi= alias)
...
- =./pi.sh --sessions=: List past sessions stored in =dockerize/pi/agent/sessions=.
